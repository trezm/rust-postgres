...
impl OwnedTransaction {
    ...
    ///
    /// This is equivalent to `OwnedTransaction`'s `Drop` implementation, but provides any error encountered to the caller.
    ...
    #[cfg(feature = "runtime")]
    #[deprecated(since = "0.6.0", note = "use OwnedTransaction::cancel_token() instead")]
    ...
    // TODO(trezm): Commented out for now
    // /// Like `Client::transaction`, but creates a nested transaction via a savepoint.
    // pub async fn transaction(&mut self) -> Result<OwnedTransaction, Error> {
    //     self._savepoint(None).await
    // }

    // /// Like `Client::transaction`, but creates a nested transaction via a savepoint with the specified name.
    // pub async fn savepoint<I>(self, name: I) -> Result<OwnedTransaction, Error>
    // where
    //     I: Into<String>,
    // {
    ...
    // async fn _savepoint(mut self, name: Option<String>) -> Result<OwnedTransaction, Error> {
